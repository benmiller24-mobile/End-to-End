/**
 * Design score — the designer-grade rubric (scorer v2), ENGINE-SIDE (AD-3).
 * Moved from evals/si/scoreKitchenV2.mjs so the generate-and-score core can
 * rank candidates with the same rubric CI ratchets on; the evals module
 * re-exports this file (one source of truth). Pure: engine output only.
 * =============================================================================
...
// The countertop physically CONTINUES over fillers and thin end panels — a 3"
// filler between two bases doesn't interrupt a landing. Pass-through items
// contribute width to the walk without counting as cabinetry.
function isCounterPassThrough(c) {
  if (FILLER_RE.test(String(c.sku || ''))) return true;
  if ((c.type === 'end_panel' || c.type === 'panel') && (c.width || 0) <= 3.01) return true;
  if (/^(FC-)?(BEP|WEP|REP|FREP)/.test(String(c.sku || '')) && (c.width || 0) <= 3.01) return true;
  return false;
}
...
function counterBeside(run, edge, dir, opts = {}) {
  let len = 0, at = edge;
  const eps = 1.0;
  for (let guard = 0; guard <= run.length + 2; guard++) {   // hard cap: never walk more items than exist
    const nxt = run.find(c => dir > 0
      ? near(c.position, at, eps)
      : near(c.position + c.width, at, eps));
    if (nxt && isCounterPassThrough(nxt)) {
      const nextAt = dir > 0 ? nxt.position + nxt.width : nxt.position;
      // overlapping/zero-width parts can move the cursor backwards within the
      // tolerance — the walk must make strict progress or stop.
      if (dir > 0 ? nextAt <= at + 0.01 : nextAt >= at - 0.01) break;
      len += nxt.width;   // counter continues over fillers / thin panels
      at = nextAt;
      continue;
    }
