...
def classify_sku(sku):
    ...
    if sku_upper.startswith('DSB'):
        return {'shapeType': SHAPE_DIAGONAL, 'zone': 'SINK_BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}

    # Right Angle corners (BL, BLSB, BLE)
    if sku_upper.startswith('BLSB') or sku_upper.startswith('BLSBE'):
        return {'shapeType': SHAPE_RIGHT_ANGLE, 'zone': 'SINK_BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}
    if sku_upper.startswith('BLE') or sku_upper.startswith('BL'):
        if not sku_upper.startswith('BND') and not sku_upper.startswith('BWD'):
            return {'shapeType': SHAPE_RIGHT_ANGLE, 'zone': 'BASE', 'defaultDepth': 24,
                    'defaultHeight': 30.5, 'yMount': 4}

    # Angle end bases
    if sku_upper.startswith('AEB') or sku_upper.startswith('ABL') or sku_upper.startswith('ABR') or sku_upper.startswith('AB2'):
        return {'shapeType': SHAPE_ANGLED_END, 'zone': 'BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}

    # Base end shelf
    if sku_upper.startswith('BES'):
        return {'shapeType': SHAPE_END_SHELF, 'zone': 'BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}

    # Blind corner bases
    if sku_upper.startswith('SBBC'):
        return {'shapeType': SHAPE_BLIND_CORNER, 'zone': 'SINK_BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}
    if sku_upper.startswith('PBBC'):
        return {'shapeType': SHAPE_BLIND_CORNER, 'zone': 'BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}
    ...
    if sku_upper.startswith('SWBC'):
        return {'shapeType': SHAPE_BLIND_CORNER, 'zone': 'UPPER', 'defaultDepth': 13,
    ...
    if sku_upper.startswith('RW'):
        return {'shapeType': SHAPE_BOX, 'zone': 'ABOVE_TALL', 'defaultDepth': 24,
                'defaultHeight': 24, 'yMount': 84}

    # Stacked wall
    if sku_upper.startswith('SW'):
        return {'shapeType': SHAPE_BOX, 'zone': 'UPPER', 'defaultDepth': 13,
                'defaultHeight': 48, 'yMount': 54}
    ...
    if sku_upper.startswith('UTWPOP') or sku_upper.startswith('UTPOP') or sku_upper.startswith('UT'):
        return {'shapeType': SHAPE_BOX, 'zone': 'TALL', 'defaultDepth': 24,
    ...
    if sku_upper.startswith('SBA'):
        return {'shapeType': SHAPE_BOX, 'zone': 'SINK_BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}
    ...
    if any(sku_upper.startswith(p) for p in ('BWRX', 'BWC', 'BWDG', 'BWS', 'BCNCWS', 'BCVXWS')):
        return {'shapeType': SHAPE_BOX, 'zone': 'BASE', 'defaultDepth': 13,
                'defaultHeight': 30.5, 'yMount': 4}
    if sku_upper.startswith('BWST'):
        return {'shapeType': SHAPE_BOX, 'zone': 'BASE', 'defaultDepth': 24,
                'defaultHeight': 30.5, 'yMount': 4}
