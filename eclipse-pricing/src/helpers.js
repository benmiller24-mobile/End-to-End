...
export function isSqIn(sku, ref) { return (SQI_REFS.has(ref) && !SQI_FIXED.has(sku)) || SQI_SKUS.has(sku); }
...
export function guessBuiltInROT(sku) {
  const s = sku.toUpperCase().replace(/\s+/g, '');
  if (/-RT/.test(s)) { if (/^SB/.test(s)) return 1; if (/^B/.test(s)) return 2; }
...
export function formatCurrency(n) { return "$" + Math.round(n || 0).toLocaleString(); }
